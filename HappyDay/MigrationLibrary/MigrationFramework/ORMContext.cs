...
{
    private readonly string _connectionString = "Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog=HappyDay;Integrated Security=True";
    ...
    public void CreateTableFromModel(Type type) // CREARE TABLE FROM MODEL
    {
        using (SqlConnection sqlConnection = new SqlConnection(_connectionString))
        {
            sqlConnection.Open();
            var tableName = type.GetCustomAttribute<Table>().TableName;
            ...
            // PRIMARY KEY
            var primaryKey = properties.Where(p => p.GetCustomAttribute<PrimaryKey>() != null).FirstOrDefault();
            var primaryKeyAttribute = primaryKey.GetCustomAttribute<PrimaryKey>();
            query.Append($"{primaryKey.Name} {GetColumnDefinition(primaryKey)} PRIMARY KEY IDENTITY,\n");

            var columns = properties.Where(p => p.GetCustomAttribute<Column>() != null).ToArray();
            // EACH COLUMN
    ...
    {
        /*
         * Пройтись по таблице и сверить со свойствами типа
         * Добавить/удалить колонки
         * цикл по свойствам - обработка по таблице
         * цикл по таблице - оставшиеся удалить
         */
        ...
        using (SqlConnection sqlConnection = new SqlConnection(_connectionString))
        {
            ...
            string query = $"SELECT * FROM {tableName} WHERE 1=0"; // Не возвращает данные, только схему
            ...
            using (SqlCommand command = new SqlCommand(query, sqlConnection))
            {
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    foreach (DataRow row in schemaTable.Rows)
                    {
                        ...
                        tableColumns.Add(columnName, sqlDataType);

                        Console.WriteLine($"Column: {columnName}, Type: {dataType}, SQL Type: {sqlDataType}, Size: {columnSize}, Nullable: {isNullable}");
                    }
                }
            }
