...
public class GetResponseBytes
{
    ...
    private static byte[]? TryGetFile(string path)
    {
        try
        {
            ...
            var staticDirectoryPath = SettingsManager.Instance.Settings.StaticDirectoryPath;
            var fileName = targetPath.Split("\\").TakeLast(1).ToList()[0];

            //OLD
            //var fn = Path.GetFileName(targetPath);
            //var ef = Directory.EnumerateFiles(staticDirectoryPath, fn, SearchOption.AllDirectories);

            // Подразумеваются уникальные названия для файлов
            string? found = Directory.EnumerateFiles(staticDirectoryPath, $"{fileName}", SearchOption.AllDirectories)
