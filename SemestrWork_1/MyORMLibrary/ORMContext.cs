...
public class ORMContext
{
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                if (reader.Read())
                {
                    for (int i = 0; i < reader.FieldCount; i++)
                    {
                        ...
                        lastIndex = (int)reader.GetValue(i);
                    }
                }
            }
        }
    ...
    public void AddToTable<T>(T entity, string tableName) where T : class
    {
        ...
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            ...
            {
                var valueSet = value[i] != null ? value[i] : string.Empty;
                command.Parameters.AddWithValue(param[i], valueSet);
            }
...
public static class ExpressionTransformer
{
    ...
    {
        var tableName = typeof(T).Name; // Имя таблицы, основанное на имени класса
        var whereClause = ParseExpression(predicate.Body);
        var limitClause = singleResult ? "TOP 1" : string.Empty;

        return $"SELECT {limitClause} * FROM {tableName} WHERE {whereClause}".Trim();
    }
}

public class LibTour
{
    [NotMapped]
