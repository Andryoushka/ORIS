...
public class ZPacketConverter
/*
 * Через активатор создавать сообщение с нужным типом и назначать поля по их id в пакета
 */
{
    private static List<Tuple<FieldInfo, byte>> GetFields(Type t)
    {
        return t.GetFields(BindingFlags.Instance |
                           ...
                           BindingFlags.Public)
        .Where(field => field.GetCustomAttribute<ZFieldAttribute>() != null)
        .Select(field => Tuple.Create(field, field.GetCustomAttribute<ZFieldAttribute>().FieldID))
        .ToList();
    }

    public static ZPacket Serialize(ZPacketType type, object obj, bool strict = false)
    {
        var t = ZPacketTypeManager.GetType(type);
        return Serialize(t.Item1, t.Item2, obj, strict);
    }

    public static ZPacket Serialize(byte type, byte subtype, object obj, bool strict = false)
    {
        ...
        var packet = ZPacket.Create(type, subtype);
    ...
    public static T Deserialize<T>(ZPacket packet, bool strict = false)
    {
